...
If your worker has a high volume of traffic, the tail might enter "sampling mode". This will cause some of your messages to be dropped and a warning to appear in your tail logs. To prevent messages from being dropped, add the optional filters listed above to reduce the volume of tail messages.

{{<Aside type="note">}}
It may take up to 1 minute for a tail to exit sampling mode after adding filters
{{</Aside>}}

If sampling persists after using filters, consider using [instant logs](https://developers.cloudflare.com/logs/instant-logs/).
